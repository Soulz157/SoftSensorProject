...
interface Props {
  ...
  dataset: Dataset
  /** When set, highlights this tag's row + column in the full heatmap table. */
  highlightTag?: string
}
 ...
 */
export function CorrelationHeatmap({ dataset, highlightTag }: Props) {
  ...
  return (
    <div className="space-y-4 rounded-xl bg-card p-4 ring-1 ring-foreground/10">
      ...
      <CorrelationMatrix dataset={dataset} highlightTag={highlightTag} />
    </div>
  )
}

...
import { mpSelectedSavedSourceIdsAtom } from '@/store/model-pipeline'
...
export function SourcePickerSheet({ open, onOpenChange }: Props) {
  const { sources: savedSources, loading } = useDataSources()
  const [selectedSources, setSelectedSources] = useAtom(dwSelectedSourcesAtom)
  ...
  const toggleSource = (id: string) => {
    setSelectedSources(prev =>
      prev.some(s => s.id === id)
        ...
        : [...prev, savedSources.find(s => s.id === id)!],
    )
    setSelectedIds(prev =>

'use client'

import { useState } from 'react'
import { Pencil } from 'lucide-react'
import { cn } from '@/lib/utils'
import {
  type DataSourceConfig,
  DEFAULT_PI_CONFIG,
  DEFAULT_SQL_CONFIG,
  DEFAULT_REST_API_CONFIG,
  DEFAULT_CSV_CONFIG,
} from '@/store/model-pipeline'
import type { SavedDataSource, DataSourceKind } from '@/lib/mock-data-sources'
import { PIConfigForm } from './pi-config-form'
import { SQLConfigForm } from './sql-config-form'
import { RestApiConfigForm } from './rest-api-config-form'
import { CSVConfigForm } from './csv-config-form'
import { AddConnectionDialog, KIND_META } from '../add-connection-dialog'

export function defaultConfigForKind(kind: DataSourceKind): DataSourceConfig {
  switch (kind) {
    case 'aveva':
      return { ...DEFAULT_PI_CONFIG }
    case 'sql':
      return { ...DEFAULT_SQL_CONFIG }
    case 'api':
      return { ...DEFAULT_REST_API_CONFIG }
    case 'csv':
      return { ...DEFAULT_CSV_CONFIG }
  }
}

/**
 * Prefill a fetch config from the Phase-2 saved source so the card shows the
 * *inherited* connection details instead of blanks. Backend `SavedDataSource`
 * only stores `host`/`dbName`/`username`, so higher-level fields (calc type,
 * method, interval) fall back to their kind defaults and stay editable.
 */
export function configFromSource(source: SavedDataSource): DataSourceConfig {
  const host = source.host.trim()
  const db = source.dbName.trim()
  switch (source.type) {
    case 'aveva':
      return {
        ...DEFAULT_PI_CONFIG,
        piServerUrl: host ? `\\\\${host}\\PI` : db,
        userName: source.username,
      }
    case 'sql':
      return {
        ...DEFAULT_SQL_CONFIG,
        connectionString: [host, db].filter(Boolean).join('/'),
      }
    case 'api':
      return {
        ...DEFAULT_REST_API_CONFIG,
        url: host ? `https://${host}` : '',
      }
    case 'csv':
      return { ...DEFAULT_CSV_CONFIG }
  }
}

/**
 * After editing a saved connection, re-derive only the identity fields
 * (endpoint/piServerUrl/connectionString/url) from the updated source while
 * preserving already-edited fetch config (calc type/basis/interval, query,
 * method/headers/timestamp/value fields). Falls back to a full
 * `configFromSource()` rebuild if the source's kind changed, since the old
 * config shape no longer applies.
 */
export function refreshIdentityFields(
  existing: DataSourceConfig | undefined,
  updated: SavedDataSource,
): DataSourceConfig {
  const rebuilt = configFromSource(updated)
  if (!existing || existing.type !== rebuilt.type) return rebuilt

  switch (existing.type) {
    case 'pi':
      return rebuilt.type === 'pi'
        ? {
            ...existing,
            piServerUrl: rebuilt.piServerUrl,
            userName: rebuilt.userName,
          }
        : rebuilt
    case 'sql':
      return rebuilt.type === 'sql'
        ? { ...existing, connectionString: rebuilt.connectionString }
        : rebuilt
    case 'rest_api':
      return rebuilt.type === 'rest_api'
        ? { ...existing, url: rebuilt.url }
        : rebuilt
    case 'csv':
    case 'influxdb':
      return existing
  }
}

interface Props {
  source: SavedDataSource
  config: DataSourceConfig
  onChange: (config: DataSourceConfig) => void
  disabled?: boolean
  /** Enables the per-card Edit button; omit to hide it (e.g. Data Studio reuse). */
  onSourceUpdate?: (updated: SavedDataSource) => void
}

export function SourceFetchConfigCard({
  source,
  config,
  onChange,
  disabled,
  onSourceUpdate,
}: Props) {
  const { icon: Icon, label } = KIND_META[source.type]
  const [editOpen, setEditOpen] = useState(false)

  return (
    <div
      className={cn(
        'space-y-3 rounded-xl bg-card p-4 ring-1 ring-foreground/10',
        disabled && 'pointer-events-none opacity-50',
      )}
    >
      <div className="flex items-center gap-2">
        <Icon className="h-4 w-4 text-muted-foreground" />
        <h3 className="text-sm font-medium text-foreground">{source.name}</h3>
        <span className="rounded-full bg-muted px-2 py-0.5 text-[11px] text-muted-foreground">
          {label}
        </span>
        {onSourceUpdate && (
          <button
            type="button"
            aria-label="Edit connection"
            disabled={disabled}
            onClick={() => setEditOpen(true)}
            className="ml-auto flex h-6 w-6 items-center justify-center rounded text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
          >
            <Pencil className="h-3.5 w-3.5" />
          </button>
        )}
      </div>

      {onSourceUpdate && (
        <AddConnectionDialog
          open={editOpen}
          onOpenChange={setEditOpen}
          sourceId={source.id}
          initialData={{
            name: source.name,
            type: source.type,
            host: source.host,
            username: source.username,
            dbName: source.dbName,
          }}
          onSave={onSourceUpdate}
        />
      )}

      {config.type === 'pi' && (
        <PIConfigForm config={config} onChange={onChange} disabled={disabled} />
      )}
      {config.type === 'sql' && (
        <SQLConfigForm
          config={config}
          onChange={onChange}
          disabled={disabled}
        />
      )}
      {config.type === 'rest_api' && (
        <RestApiConfigForm
          config={config}
          onChange={onChange}
          disabled={disabled}
        />
      )}
      {config.type === 'csv' && (
        <CSVConfigForm
          config={config}
          onChange={onChange}
          disabled={disabled}
        />
      )}
    </div>
  )
}
